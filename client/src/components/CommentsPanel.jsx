 ...
 */
/**
 * CommentsPanel — shows AI review comments as scrollable cards.
 *
 * Props:
 *   comments         — array of { line, text, severity, category }
 *   reviewInProgress — boolean
 ...
 *   onToggle         — called when the header toggle is clicked
 *   embedded         — when true, renders only the card list (no toggle header),
 *                      used when hosted inside the tabbed bottom panel
 */
export default function CommentsPanel({ comments, reviewInProgress, open, height, onToggle, embedded = false }) {
  // In embedded mode we just render the card content directly.
  if (embedded) {
    return (
          ...
          {comments.length === 0 ? (
            <div className="h-full flex items-center justify-center">
              <p className="text-xs text-gray-700">
                {reviewInProgress ? 'Waiting for comments…' : 'No review yet — use "Request AI Review" in the sidebar'}
              </p>
            </div>
          ) : (
  ...
  // Standalone mode: includes toggle header (legacy, kept for backward compat).
  return (
    <div
    ...
    >
      ...
      >
        ...
        {!reviewInProgress && comments.length > 0 && (
          <span className="text-xs text-gray-600">{comments.length} comment{comments.length !== 1 ? 's' : ''}</span>
        )}
      </button>
      {open && (
        <div className="flex-1 overflow-x-auto overflow-y-hidden">
