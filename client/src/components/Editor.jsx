...
// ─── Cursor colors ────────────────────────────────────────────────────────────
// CSS hex values matching the Tailwind palette used in UserList.jsx so each
// user gets the same color everywhere in the UI.
const CURSOR_COLORS = [
  '#6366f1', // indigo-500
  '#10b981', // emerald-500
  '#f43f5e', // rose-500
...
// ─── Remote cursor DOM widget ─────────────────────────────────────────────────
// CodeMirror renders this as an inline element at the cursor's character offset.
// It draws a thin colored vertical bar (the caret) and a name label above it.
class RemoteCursorWidget extends WidgetType {
  ...
  toDOM() {
    // A 2px wide inline block — acts as the cursor caret line.
    // Using width+background instead of border-left on an absolute element
    // keeps it from stretching outside the line height.
...
const remoteCursorField = StateField.define({
  ...
  // Derive a DecorationSet directly from the cursor map.
...
// ─── Annotation to tag remote-edit transactions ───────────────────────────────
...
export default function Editor({ onLocalChange, onCursorMove, onReady }) {
  ...
  useEffect(() => {
    ...
    onReady?.({
      ...
      // Remove a user's cursor widget (called when they leave the room).
      removeCursor: (userId) => {
